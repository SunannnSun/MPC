...
def simulate_quadrotor(x0, tf, quadrotor, use_mpc=True):
  ...
  while np.linalg.norm(np.array(x[-1][0:2])) > 1e-3 and t[-1] < tf:
    current_time = t[-1]
    current_x = x[-1]
    current_u_command = u[-1]
    
    if use_mpc:
      current_u_command, x_prediction = quadrotor.compute_mpc_feedback(current_x, current_u_command)
    else:
  ...
  u = np.array(u)
  t = np.array(t)
  return x, u, t, x_prediction_list
